package NcnnTencent.CloudDetect;
...
/**
 * MainCloudActivity - 云端检测主函数
 * 负责配置网络流参数，启动和停止网络流检测
 */
public class MainCloudActivity extends Activity implements NetworkVideoManager.NetworkVideoCallback {
    private static final String TAG = "MainCloudActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        jniBridge.loadModel(getAssets(), modelId, cpuGpu, inputSize);
    ...
    private void refreshLog() {
        ...
        if (isDetecting) {
            // 检测模式下显示检测结果
            DetectSummary summary = jniBridge.getDetectSummary();
    ...
    private final Runnable monitorRefresher = new Runnable() {
        ...
        public void run() {
            ...
            // 更新监控信息
            if (isPreviewing && jniBridge != null) {
                DetectSummary summary = jniBridge.getDetectSummary();
                if (summary != null) {
                    if (tvAllValue != null) {
    ...
    private void updateInferenceParams() {
        if (jniBridge != null) {
            jniBridge.setThreshold(threshold);
            jniBridge.setNms(nmsThreshold);
            jniBridge.setTrackEnabled(trackEnabled);
            jniBridge.setShaderEnabled(shaderEnabled);
