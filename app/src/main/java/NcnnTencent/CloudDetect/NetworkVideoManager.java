...
public class NetworkVideoManager {
    ...
    private NcnnTencent.common.JniBridge jniBridge;
    ...
    public NetworkVideoManager(Context context) {
        ...
        this.jniBridge = new NcnnTencent.common.JniBridge();
    }
    ...
    public void startNetworkStream(String url, int inputSize, int modelId, int cpuGpu) {
        try {
            ...
            // 启动JNI层的网络流处理（可能抛出异常）
            try {
                jniBridge.startNetworkVideoStream(url, inputSize, modelId, cpuGpu, this);
            } catch (Exception e) {
                Log.e(TAG, "JNI调用异常", e);
